...
const Designer2DCanvas: React.FC<Designer2DCanvasProps> = ({
  ...
  const [currentWall, setCurrentWall] = useState<fabric.Object | null>(null);
  ...
  const drawWall = useCallback((wall: Wall) => {
    ...
    // Draw wall as a rotated rectangle for correct thickness
    const wallRect = new fabric.Rect({
      left: wall.start.x,
      top: wall.start.y - wall.thickness / 2,
      width: length,
    ...
    (wallRect as any).name = 'wall';
    (wallRect as any).wallData = wall;

    // Add wall thickness visualization
    const wallBackgroundRect = new fabric.Rect({
      left: Math.min(wall.start.x, wall.end.x) - wall.thickness / 2,
      top: Math.min(wall.start.y, wall.end.y) - wall.thickness / 2,
    ...
    (wallBackgroundRect as any).name = 'wall-background';

    fabricCanvasRef.current.add(wallBackgroundRect, wallRect);
    fabricCanvasRef.current.sendObjectToBack(wallBackgroundRect);
    ...
    try {
      ...
      canvas.on('mouse:move', (options) => {
        if (selectedTool === 'wall' && isDrawingWall && wallStartPoint) {
          ...
          // Remove previous preview wall
          if (currentWall) {
            canvas.remove(currentWall);
          }

          // Calculate angle and length
          const dx = snappedPoint.x - wallStartPoint.x;
          const dy = snappedPoint.y - wallStartPoint.y;
          const length = Math.sqrt(dx * dx + dy * dy);
          const angle = Math.atan2(dy, dx) * (180 / Math.PI);

          // Draw preview wall as a rotated rectangle
          const previewWall = new fabric.Rect({
            left: wallStartPoint.x,
            top: wallStartPoint.y - wallThickness / 2,
            width: length,
    ...
    <div className="flex h-full bg-gray-50">
      ...
      <div className="flex-1 relative">
        <canvas
          ...
          style={{ backgroundColor: '#000000' }} // <-- Set to black
        />
